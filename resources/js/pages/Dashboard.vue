<template>
    ...
    <div v-else
         class="flex flex-no-wrap bg-gray-200 mt-10">
        <!-- Sidebar starts -->
                ...
                <ul class="mt-12">
                    <li class="flex w-full justify-between text-gray-300 hover:text-gray-500 cursor-pointer items-center mb-6">
                        <div class="flex items-center">
                            <span class="text-sm ml-2">Name: {{ user.name }}</span>
                        </div>
                    </li>
                    <li class="flex w-full justify-between text-gray-300 hover:text-gray-500 cursor-pointer items-center mb-6">
                        <div class="flex items-center">
                            <span class="text-sm ml-2">Lastname: {{ user.lastName }}</span>
                        </div>
                    </li>
                    <li class="flex w-full justify-between text-gray-300 hover:text-gray-500 cursor-pointer items-center mb-6">
                        <div class="flex items-center">
                            <span class="text-sm ml-2">Grade: {{ user.grade.name }}</span>
                        </div>
                    </li>
                    <li class="flex w-full justify-between text-gray-300 hover:text-gray-500 cursor-pointer items-center mb-6">
                        <div class="flex items-center">
                            <span class="text-sm ml-2">On Duty: {{ dutyCount }}</span>
                        </div>
                    </li>
                    <li class="flex w-full justify-between text-gray-300 hover:text-gray-500 cursor-pointer items-center mb-6">
                        <div class="flex items-center">
                            <span class="text-sm ml-2">Danger Level: {{ dangerLevel.name }}</span>
                        </div>
                    </li>
                    <li class="flex w-full justify-between text-gray-300 hover:text-gray-500 cursor-pointer items-center mb-6">
                        <div class="flex items-center">
                            <span class="text-sm ml-2">PWC: {{ dispatchUser.name }} {{dispatchUser.last_name}}</span>
                        </div>
                    </li>
                </ul>
...
export default {
    ...
    async mounted() {
        ...
        await this.getCurrentDispatch();
        this.isLoading = false;
    },

    methods: {
        setDuty: async function () {
            await axios.post('api/duty/set').then((data) => {
                this.onDuty = data.data
            }).catch((error) => {
                console.log(error)
            }).finally(()=>{
                this.getAllDuties();
        ...
        getCurrentDispatch: async function(){
            await axios.get('api/dispatch/current').then((data) => {
                if (data.data==false){
                    this.dispatchUser['name'] = 'No';
                    this.dispatchUser['last_name'] = 'PWC';
                }
                else{
                    this.dispatchUser = data.data;
                }
            })
        },
    },
